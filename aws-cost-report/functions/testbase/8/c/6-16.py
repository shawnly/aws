...
def create_cost_visualization(df, title, account_id, start_date, end_date, is_organization=False):
    ...
    # Create main chart subplot (takes most of the space)
    ...
    # Count services with cost > 0
    service_count = len(df[df[cost_column] > 0][service_column].unique())
    
    # Create summary table
    table_ax = plt.subplot2grid((4, 1), (0, 0))
    table_ax.axis('off')  # Hide axes for table
    
    # Table data
    table_data = [
        ['Total Cost', 'Average Monthly Cost', 'Service Count'],
    ...
    for i in range(3):
        table[(0, i)].set_facecolor('#4472C4')  # Blue header
        table[(0, i)].set_text_props(weight='bold', color='white')
    
    # Style data row (second row)
    for i in range(3):
        table[(1, i)].set_facecolor('#F2F2F2')  # Light gray
