...
def create_cost_visualization(df, title, account_id, start_date, end_date, is_organization=False):
    ...
    for i, service in enumerate(pivot_data.columns):
        ...
        # Add cost amounts on each segment
        for j, cost in enumerate(service_costs):
            if cost > 0:  # Only show label if there's actual cost
                segment_center = bottom[j] + cost / 2
                max_monthly_total = pivot_data.sum().max()
                
                # Show label if segment is large enough (lowered threshold to 1% and added minimum dollar amount)
                if cost > max(max_monthly_total * 0.01, 50):  # 1% of max monthly total OR at least $50
                    # Determine text color based on segment size and background
                    text_color = 'white' if cost > max_monthly_total * 0.05 else 'black'
                    
                    # Add white outline for better visibility
                    text_path_effects = [path_effects.withStroke(linewidth=2, foreground='white')] if text_color == 'black' else None
                    
                    ax.text(j, segment_center, f'${cost:,.0f}', 
    ...
    if not refunds_data.empty and refunds_data.abs().sum() > 0:
        ...
        for j, refund in enumerate(refunds_data.values):
            if refund < 0:  # Only show label if there's actual refund
                ax.text(j, refund / 2, f'${abs(refund):,.0f}', 
    ...
    for i, (month, net_total) in enumerate(net_totals.items()):
        ...
        if positive_total > 0:
            ...
            ax.text(i, positive_total + positive_total * 0.01, f'Net: ${net_total:,.0f}', 
