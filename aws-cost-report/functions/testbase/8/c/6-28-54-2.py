...
def get_organization_cost_by_account(ce_client, start_date, end_date)::
    """Get organization-wide cost aggregated by linked account with pagination"""
    # Validate and format dates
    start_date_formatted = validate_and_format_date(start_date, "start_date")
    end_date_formatted = validate_and_format_date(end_date, "end_date")
    
    # Initial request - group by LINKED_ACCOUNT instead of SERVICE
    response = ce_client.get_cost_and_usage(
        TimePeriod={
            'Start': start_date_formatted,
            'End': end_date_formatted
...
def process_top_accounts_data(response, accounts_info):
    """Process the top accounts cost data into a DataFrame"""
    results = []
    
    # Create account ID to name mapping
    account_mapping = {acc['id']: acc['name'] for acc in accounts_info}
...
def save_top_accounts_report(df, start_date, end_date):
    """Save top accounts report to Excel file"""
    ...
    # Create filename
    filename = f"{directory}/aws-top-accounts-report-{start_date}_to_{end_date}.xlsx"
    
    # Save to Excel
    with pd.ExcelWriter(filename, engine='xlsxwriter') as writer:
        # Save detailed data to first sheet
        df.to_excel(writer, sheet_name='Account Details', index=False)
        
        # Create a pivot table summarizing total costs by account across all months
        account_summary = df.pivot_table(
            index=['Account ID', 'Account Name'],
            values=['Total Amortized Cost ($)', 'Total Unblended Cost ($)', 'Total Refund ($)'],
            aggfunc='sum'
        ).reset_index()
        
        # Sort by highest cost and get top 10
        account_summary = account_summary.sort_values('Total Amortized Cost ($)', ascending=False)
        top_10_accounts = account_summary.head(10).copy()
        
        # Add rank column
        top_10_accounts.insert(0, 'Rank', range(1, len(top_10_accounts) + 1))
        
        # Save top 10 to second sheet
        top_10_accounts.to_excel(writer, sheet_name='Top 10 Accounts', index=False)
        
        # Create pivot table by month and account (top 10 only)
        top_10_account_ids = top_10_accounts['Account ID'].tolist()
        df_top_10 = df[df['Account ID'].isin(top_10_account_ids)]
        
        monthly_pivot = df_top_10.pivot_table(
            index=['Account ID', 'Account Name'],
            columns=['Month'],
        ...
        # Save to third sheet
        monthly_pivot.to_excel(writer, sheet_name='Top 10 Monthly Breakdown', index=False)
        ...
        rank_format = workbook.add_format({'align': 'center', 'bold': True})
        
        # Get the worksheet objects
        worksheet2 = writer.sheets['Top 10 Accounts']
        
        # Set column widths and formats for the Top 10 Accounts sheet
        worksheet2.set_column('A:A', 8, rank_format)   # Rank
        worksheet2.set_column('B:B', 15)               # Account ID
        worksheet2.set_column('C:C', 30)               # Account Name
        worksheet2.set_column('D:F', 18, currency_format)  # Cost columns
    
    print(f"Top accounts report saved to {filename}")
    return filename
    """Get the appropriate end date for display purposes (charts, titles)"""
    end_date_obj = datetime.strptime(end_date, '%Y-%m-%d')
...
def main():
    ...
    # Generate top accounts report
    print("Generating top 10 accounts report...")
    accounts_info = get_all_linked_accounts(ce_client, start_date, end_date)
    top_accounts_response = get_organization_cost_by_account(ce_client, start_date, end_date)
    top_accounts_df = process_top_accounts_data(top_accounts_response, accounts_info)
    save_top_accounts_report(top_accounts_df, start_date, end_date)
