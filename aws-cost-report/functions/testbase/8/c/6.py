...
def create_cost_visualization(df, title, account_id, start_date, end_date, is_organization=False):
    """Create AWS Cost Explorer style visualization"""
    ...
    # Filter out negative costs (refunds) for cleaner visualization
    df_positive = df[df[cost_column] >= 0].copy()
    
    # Group by month and service, sum the costs
    monthly_costs = df_positive.groupby([month_column, service_column])[cost_column].sum().reset_index()
    
    # Get top 9 services overall
    ...
    for i, service in enumerate(pivot_data.columns):
        ...
        bars.append(ax.bar(pivot_data.index, pivot_data[service], bottom=bottom, 
                          label=service, color=color, alpha=0.8))
        bottom += pivot_data[service]
    
    # Add total cost labels on top of each bar
    for i, month in enumerate(pivot_data.index):
        total_cost = pivot_data.loc[month].sum()
        if total_cost > 0:
            ax.text(i, total_cost + total_cost * 0.01, f'${total_cost:,.0f}', 
