...
def get_cost_and_usage(ce_client, start_date, end_date, account_id=None):
    """Get cost and usage data from Cost Explorer with proper pagination handling"""
    # Validate and format dates
    start_date_formatted = validate_and_format_date(start_date, "start_date")
    end_date_formatted = validate_and_format_date(end_date, "end_date")
    ...
    while True:
        ...
        # If this is the first page, initialize our results
        if not all_periods:
            all_periods = response['ResultsByTime']
        else:
            # Merge groups from this page with existing results
            for i, period in enumerate(response['ResultsByTime']):
                if i < len(all_periods):
                    all_periods[i]['Groups'].extend(period['Groups'])
                else:
        ...
        print(f"Fetched page with {len(response['ResultsByTime'])} periods, continuing...")
    
    # Consolidate duplicate services within each period
    for period in all_periods:
        ...
        for group in period['Groups']:
            ...
            # Sum up the costs for this service
            service_costs[service_name]['AmortizedCost']['Amount'] = str(
                float(service_costs[service_name]['AmortizedCost']['Amount']) + 
                float(metrics['AmortizedCost']['Amount'])
            )
            service_costs[service_name]['UnblendedCost']['Amount'] = str(
                float(service_costs[service_name]['UnblendedCost']['Amount']) + 
                float(metrics['UnblendedCost']['Amount'])
            )
            service_costs[service_name]['UsageQuantity']['Amount'] = str(
                float(service_costs[service_name]['UsageQuantity']['Amount']) + 
                float(metrics['UsageQuantity']['Amount'])
            )
        
        # Rebuild the groups with consolidated data
        period['Groups'] = []
        for service_name, metrics in service_costs.items():
            period['Groups'].append({
                'Keys': [service_name],
                'Metrics': metrics
            })
    
    # Return the consolidated results in the same format as the original API response
    return {
        'ResultsByTime': all_periods,
...
def get_organization_cost_by_service(ce_client, start_date, end_date):
    """Get organization-wide cost aggregated by service with proper pagination handling"""
    # Validate and format dates
    start_date_formatted = validate_and_format_date(start_date, "start_date")
    end_date_formatted = validate_and_format_date(end_date, "end_date")
    ...
    while True:
        ...
        print(f"Organization summary: Fetched page with {len(response['ResultsByTime'])} periods, continuing...")
