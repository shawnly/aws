...
def get_aws_cost_data(start_date, end_date):
    ...
    try:
        ...
        # Query by both service and account
...
def create_cost_dashboard(df, output_dir='aws_cost_report', 
                         start_date=None, end_date=None, file_suffix=""):
    ...
    # Get AWS account ID if available from the data
    if 'Account' in df.columns and not df['Account'].empty:
        # If we have account data (for an individual account report)
        account_id = df['Account'].iloc[0]
        # For the summary report, use a different label
        if file_suffix == "_summary":
    ...
    excel_path = os.path.join(output_dir, f'aws_cost_data{file_suffix}{date_range_suffix}.xlsx')
    ...
    # Save the chart
    chart_file = os.path.join(output_dir, f'aws_cost_report{file_suffix}{date_range_suffix}.png')
    plt.savefig(chart_file, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {chart_file}")
    
    # Also save the data to Excel
    excel_file = os.path.join(output_dir, f'aws_cost_report{file_suffix}{date_range_suffix}.xlsx')
...
def main():
    ...
    try:
        ...
        for account in accounts:
            ...
            # Get per-account file suffix
            account_suffix = f"_{account}"
            
            # Create dashboard for this account
            create_cost_dashboard(account_df, OUTPUT_DIR, start_date, end_date, account_suffix)
        
        # Create a summary report across all accounts
        print("\nCreating summary report across all accounts...")
        create_cost_dashboard(df, OUTPUT_DIR, start_date, end_date, "_summary")
