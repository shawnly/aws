...
class AWSCostReportGenerator:
    ...
    def get_cost_data(self, account_id, start_date, end_date, month_index=0):
        ...
        try:
            # Get total cost using AmortizedCost metric
            ...
            # Extract total cost using AmortizedCost
            total_cost = 0
            if (total_response.get('ResultsByTime') and 
                total_response['ResultsByTime'][0].get('Total') and 
                total_response['ResultsByTime'][0]['Total'].get('AmortizedCost')):
                total_cost = float(total_response['ResultsByTime'][0]['Total']['AmortizedCost']['Amount'])
            
            # Get service breakdown using UnblendedCost metric
            ...
            # Parse service costs
            services = []
            if (service_response.get('ResultsByTime') and 
                service_response['ResultsByTime'][0].get('Groups')):
            ...
            # Return both the amortized total cost and unblended service costs
            return {
    ...
    def _generate_mock_cost_data(self, account_id, month_index=0):
        ...
        for i, service in enumerate(selected_services):
            ...
            service_total_cost += cost
        
        # Sort by cost (descending)
        services.sort(key=lambda x: x['cost'], reverse=True)
        
        # For mock data, add a small difference between amortized cost and sum of unblended costs
        # to simulate the real-world scenario where these values differ
        amortized_total = service_total_cost * random.uniform(0.95, 1.05)
        
        return {
            'total_cost': amortized_total,
            'services': services
        }
    ...
    def generate_cost_report(self, account_id, account_name, date_ranges):
        ...
        for i, date_range in enumerate(date_ranges):
            ...
            print(f"      Total cost (Amortized): ${cost_data['total_cost']:.2f}, Services: {len(cost_data['services'])}")
        ...
        for month_data in monthly_cost_data:
            ...
            # Add a row for the total amortized cost
            csv_data.append({
                'month': month,
                'service': 'TOTAL (Amortized)',
                'cost': cost_data['total_cost']
            })
            
            # Add rows for each service (Unblended costs)
    ...
    def _create_multi_month_cost_report(self, account_id, account_name, monthly_cost_data, output_file):
        ...
        for month_data in monthly_cost_data:
            ...
            # Calculate total without refunds (using unblended costs here for visualization)
        ...
        # Calculate summary statistics - using amortized costs for totals
        ...
        for month_idx, month_data in enumerate(monthly_cost_data):
            ...
            # Add amortized total cost label at the top of the bar
            amortized_cost = month_data['cost_data']['total_cost']
            # Calculate the difference from the visualization total (unblended costs)
            unblended_total = bottoms[month_idx] + (refund_amount if refund_amount < 0 else 0)
            
            plt.text(month_idx, bottoms[month_idx] * 1.02, 
                    f'${amortized_cost:.2f} (Amortized)', 
        ...
        # Create a table for the summary stats - using the amortized total cost
        col_labels = ['Total Cost (Amortized)', 'Average Monthly Cost', 'Service Count']
        table_data = [
    ...
    def generate_organization_summary(self, accounts, date_ranges):
        ...
        try:
            ...
            for i, date_range in enumerate(date_ranges):
                ...
                # Get costs for all accounts in a single API call using AmortizedCost
        ...
        for month_data in monthly_summary_data:
            ...
            for account in month_data['account_costs']:
                csv_data.append({
                    ...
                    'cost': account['cost'],
                    'cost_type': 'Amortized'  # Indicate this is amortized cost
                })
    ...
    def _create_multi_month_organization_summary(self, monthly_summary_data, output_file):
        ...
        plt.text(0.5, 0.5, f'AWS Organization Cost Summary (Amortized Costs)\n{date_range}', 
                 fontsize=16, fontweight='bold', ha='center', va='center')
        ...
        col_labels = ['Total Cost (Amortized)', 'Average Monthly Cost', 'Account Count']
        table_data = [
