...
class AWSCostDataRetriever:
    ...
    def get_account_cost_data(self, account_id, start_date, end_date, month_index=0):
        ...
        if self.use_mock_data:
            ...
            # Use account_id and month_index as seeds for reproducible randomness
            import random
            ...
            selected_services = random.sample(self.mock_services, min(5, len(self.mock_services)))
            
            for service in selected_services:
                # Use a deterministic formula for costs
                cost = 100 + (int(account_id) % 10) * 100 + month_index * 50 + random.uniform(10, 100)
    ...
    def save_account_cost_data(self, account_id, account_name, date_ranges, cost_data_by_month):
        """
        Save account cost data to an Excel file using the original script's directory structure.
        ...
        first_month_end = date_ranges[0]['end']
        last_month_start = date_ranges[-1]['start']
        ...
        # Create a Pandas Excel writer with openpyxl as the engine
        with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
            # Create a sheet for the overall period
            all_data = []
            ...
            all_df.to_excel(writer, sheet_name='All Months', index=False)
            ...
            for i, month_data in enumerate(cost_data_by_month):
                ...
                sheet_name = f"{month} ({month_start} to {month_end})"
                # Make sure sheet name is not too long for Excel
                if len(sheet_name) > 31:
                    sheet_name = month[:31]  # Excel limit is 31 characters
                month_df.to_excel(writer, sheet_name=sheet_name, index=False)
        
        print(f"Account cost data saved to {excel_file}")
        return excel_file
    
    def save_organization_cost_data(self, accounts, date_ranges, monthly_cost_data):
        """
        Save organization-wide cost data to an Excel file using the original script's directory structure.
        ...
        # Create a Pandas Excel writer with openpyxl as the engine
        with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
            # Prepare data for the overall sheet
            all_data = []
        ...
        print(f"Organization cost summary saved to {excel_file}")
        return excel_file
