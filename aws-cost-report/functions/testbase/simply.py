...
# Use a fixed colormap for services
SERVICE_COLORS = [
...
def create_cost_dashboard(df, output_dir='aws_cost_report', 
    ...
    # Get account ID or default to "aws" if not available
    account_name = "aws"
    ...
    fig.text(0.5, 0.95, f'AWS Monthly Cost by Service{date_range_text}', 
    ...
    # Save the chart
    chart_file = os.path.join(output_dir, f'aws_cost_report{date_range_suffix}.png')
    plt.savefig(chart_file, dpi=300, bbox_inches='tight')
    print(f"Chart saved to: {chart_file}")
    
    # Also save the data to Excel
    excel_file = os.path.join(output_dir, f'aws_cost_report{date_range_suffix}.xlsx')
