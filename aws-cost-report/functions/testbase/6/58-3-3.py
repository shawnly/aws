...
class AWSCostReportGenerator:
    ...
    def generate_organization_service_summary(self, accounts, date_ranges):
        """Generate a summary report of services across the entire organization across multiple months."""
        print("Generating organization service summary...")
        ...
        output_file = os.path.join(
            output_path,
            f"aws-service-summary-{start_date}-{end_date}.png"
        )
        ...
        for i, date_range in enumerate(date_ranges):
            ...
            if self.use_mock_data:
                # Aggregate mock data across all accounts for services
                service_costs = {}
                ...
                services = [{'service': service, 'cost': cost} for service, cost in service_costs.items()]
                
            else:
                try:
                    # Try to get all service costs for this month in a single API call
                    response = self.ce_client.get_cost_and_usage(
                        TimePeriod={
                            'Start': date_range['start'],
                            'End': date_range['end']
                    ...
                    print(f"Failed to get all service costs in one call: {str(e)}")
                    print("Falling back to per-account data aggregation (slower method)...")
                    
                    # Aggregate data by service across all accounts
                    service_costs = {}
            ...
            # Sort by cost (descending) but handle refunds
            refund_services = [s for s in services if 'refund' in s['service'].lower()]
            other_services = [s for s in services if 'refund' not in s['service'].lower()]
            ...
            monthly_service_data.append({
                'month': month_date,
                'total_cost': total_cost,
                'services': sorted_services
            })
            
            print(f"  Processed {len(services)} services with total cost: ${total_cost:.2f}")
        
        # Create the service summary visualization
        print("Creating organization service summary visualization...")
        self._create_multi_month_service_summary(
            monthly_service_data,
            output_file
        )
        ...
        csv_file = os.path.join(
            output_path,
            f"aws-service-summary-{start_date}-{end_date}.csv"
        )
        
        # Prepare CSV data
        csv_data = []
        for month_data in monthly_service_data:
            month = month_data['month']
        ...
        print(f"Organization service summary completed and saved to {output_file}")
        return output_file

    def _create_multi_month_service_summary(self, monthly_service_data, output_file):
        """Create an organization service summary visualization with one bar per month."""
        # Set up the figure with improved proportions
        plt.figure(figsize=(16, 12))  # Wider figure to accommodate legend better
        ...
        plt.sca(title_ax)
        title_ax.axis('off')  # Hide the axes
        date_range = f"{monthly_service_data[0]['month']} to {monthly_service_data[-1]['month']}"
        plt.text(0.5, 0.5, f'AWS Organization Service Cost Summary\n{date_range}', 
                fontsize=16, fontweight='bold', ha='center', va='center')
        ...
        all_services = set()
        for month_data in monthly_service_data:
            # Only include services with meaningful costs (more than $0.01) and exclude refunds temporarily
            services = [service['service'] for service in month_data['services'] 
                    if service['cost'] > 0.01 and 'refund' not in service['service'].lower()]
            all_services.update(services)
        
        # Calculate summary statistics
        total_cost = sum(month_data['total_cost'] for month_data in monthly_service_data)
        avg_monthly_cost = total_cost / len(monthly_service_data)
        service_count = len(all_services)
        ...
        # Prepare data for the stacked bar chart
        months = [data['month'] for data in monthly_service_data]
        x_positions = np.arange(len(months))
        
        # Process each month's data for calculating refunds and maximums
        for month_data in monthly_service_data:
            # Process refunds separately
            refund_services = [s for s in month_data['services'] 
                            if s['service'].lower() == 'refund' or 'refund' in s['service'].lower()]
            other_services = [s for s in month_data['services'] 
                            if s['service'].lower() != 'refund' and 'refund' not in s['service'].lower()]
        ...
        # Find maximum cost and refund to set y-axis scale properly
        max_positive_cost = max(month_data['total_without_refunds'] for month_data in monthly_service_data)
        min_refund = min(month_data['refund_amount'] for month_data in monthly_service_data)
        ...
        # Process each month's data for positive costs (non-refunds)
        for month_idx, month_data in enumerate(monthly_service_data):
            # Filter services with significant costs (more than $0.01)
            significant_services = [s for s in month_data['other_services'] 
                                if s['cost'] > 0.01 and 'refund' not in s['service'].lower()]
            
            # Get the top 10 services for this month
            top_services = significant_services[:10]
            
            # Calculate the "Other" category if needed - sum services beyond top 10
            # and services with negligible costs (≤ $0.01)
            other_services_cost = sum(service['cost'] for service in month_data['other_services'][10:] 
                                    if 'refund' not in service['service'].lower())
            negligible_cost = sum(service['cost'] for service in month_data['other_services'] 
                                if service['cost'] <= 0.01 and 'refund' not in service['service'].lower())
            other_services_cost += negligible_cost
            ...
            if other_services_cost > 0.01:  # Only show "Other" if it's meaningful
                ...
                # Add to legend handles only once
                if 'Other Services' not in [h.get_label() for h in handles]:
                    handles.append(plt.Rectangle((0,0), 1, 1, color='#cccccc', label='Other Services'))
            ...
            # Add NET total cost label at the top of the bar (including refunds)
            net_cost = month_data['total_cost']
            plt.text(month_idx, bottoms[month_idx] * 1.02, f'${net_cost:.2f}', 
                    ha='center', va='bottom', fontsize=10, fontweight='bold', color='black')
        ...
        print(f"Organization service summary saved to {output_file}")


    def export_raw_cost_data(self, accounts, date_ranges):
        """
        Export raw cost data for all accounts into CSV files before generating reports.
        Uses the same directory structure as the cost reports.
        
        Args:
            accounts: List of account dictionaries with 'id' and 'name' keys
            date_ranges: List of date range dictionaries
        
        Returns:
            Dictionary mapping account IDs to their raw data CSV files
        """
        print("Exporting raw cost data for all accounts...")
        
        # Get year and month from the last date range for the output directory
        last_date_range = date_ranges[-1]
        year = last_date_range['year']
        month = last_date_range['month']
        
        # Generate the full date range for filename
        start_date = date_ranges[0]['start']
        end_date = date_ranges[-1]['end']
        
        # Create a timestamp for the organization summary files
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Create consolidated organization output path
        org_output_path = os.path.join(
            self.output_dir,
            "organization_summary",
            year,
            f"end-of-{month}",
        ...
        os.makedirs(org_output_path, exist_ok=True)
        
        # Generate consolidated CSV for all accounts across all months
        all_accounts_csv = os.path.join(org_output_path, f"all_accounts_raw_data_{start_date}_{end_date}.csv")
        consolidated_data = []
        ...
        for account in accounts:
            account_id = account['id']
            account_name = account['name']
            print(f"Exporting raw data for account {account_id} ({account_name})...")
            
            # Create account-specific output path (same structure as cost reports)
            account_output_path = os.path.join(
                self.output_dir,
                account_id,
                year,
                f"end-of-{month}",
            ...
            os.makedirs(account_output_path, exist_ok=True)
            ...
            for date_range in date_ranges:
                ...
                # Get cost data for this account and date range
                cost_data = self.get_cost_data(account_id, start, end)
            ...
            # Create account-specific CSV file
            if account_data:
                account_csv = os.path.join(account_output_path, f"account_{account_id}_raw_data_{start_date}_{end_date}.csv")
                df = pd.DataFrame(account_data)
                df.to_csv(account_csv, index=False)
                account_csv_files[account_id] = account_csv
                print(f"  Raw data for account {account_id} saved to {account_csv}")
                
                # Add monthly totals to help with debugging
                monthly_totals_csv = os.path.join(account_output_path, f"account_{account_id}_monthly_totals_{start_date}_{end_date}.csv")
                monthly_totals = []
                
                for date_range in date_ranges:
                    month_name = date_range['month_name']
                    month_data = [item for item in account_data if item['month'] == month_name]
                    total_cost = sum(item['cost'] for item in month_data)
                ...
                df_totals.to_csv(monthly_totals_csv, index=False)
        
        # Create service summary output path
        service_output_path = os.path.join(
            self.output_dir,
            "organization_service_summary",
            year,
            f"end-of-{month}",
        ...
        os.makedirs(service_output_path, exist_ok=True)
        
        # Create consolidated CSV file
        if consolidated_data:
            # Save to both organization summary and service summary directories
            df_all = pd.DataFrame(consolidated_data)
            df_all.to_csv(all_accounts_csv, index=False)
            
            service_csv = os.path.join(service_output_path, f"all_services_raw_data_{start_date}_{end_date}.csv")
            df_all.to_csv(service_csv, index=False)
            
            print(f"Consolidated raw data saved to {all_accounts_csv} and {service_csv}")
            
            # Create organization monthly totals for comparison with AWS console
            org_monthly_totals_csv = os.path.join(org_output_path, f"organization_monthly_totals_{start_date}_{end_date}.csv")
            service_monthly_totals_csv = os.path.join(service_output_path, f"organization_monthly_totals_{start_date}_{end_date}.csv")
            
            org_monthly_totals = []
            
            for date_range in date_ranges:
                month_name = date_range['month_name']
                month_data = [item for item in consolidated_data if item['month'] == month_name]
                total_cost = sum(item['cost'] for item in month_data)
                
                # Add service breakdown for this month
                services = {}
                ...
                # Add top services to the monthly total row
                monthly_total = {
                    'month': month_name,
                    'total_cost': total_cost
                }
                ...
                # Add top 10 services
                for i, (service, cost) in enumerate(sorted_services[:10]):
                    monthly_total[f'top_{i+1}_service'] = service
                    monthly_total[f'top_{i+1}_cost'] = cost
                
                org_monthly_totals.append(monthly_total)
            
            df_org_totals = pd.DataFrame(org_monthly_totals)
            df_org_totals.to_csv(org_monthly_totals_csv, index=False)
            df_org_totals.to_csv(service_monthly_totals_csv, index=False)
            print(f"Organization monthly totals saved to {org_monthly_totals_csv} and {service_monthly_totals_csv}")
        
        # Generate summary statistics to help with debugging
        summary_csv = os.path.join(org_output_path, f"cost_summary_stats_{start_date}_{end_date}.csv")
        service_summary_csv = os.path.join(service_output_path, f"cost_summary_stats_{start_date}_{end_date}.csv")
        ...
        try:
            ...
            profile = session.profile_name if hasattr(session, 'profile_name') else "default"
            region = session.region_name
            
            if self.use_mock_data:
                summary_data.append({
                    'key': 'data_source',
                    'value': 'Mock data (no actual AWS API calls)'
                })
            else:
                summary_data.append({
                    'key': 'data_source',
                    'value': f'AWS Cost Explorer API (Profile: {profile}, Region: {region})'
                })
        except Exception as e:
            summary_data.append({
                'key': 'data_source',
                'value': f'AWS Cost Explorer API (Error getting profile: {str(e)})'
            })
        ...
        # Add total cost across all accounts and dates
        if consolidated_data:
            total_cost = sum(item['cost'] for item in consolidated_data)
            summary_data.append({
                'key': 'total_cost',
                'value': total_cost
            })
        ...
        if consolidated_data:
            for date_range in date_ranges:
                month_name = date_range['month_name']
                month_data = [item for item in consolidated_data if item['month'] == month_name]
                month_cost = sum(item['cost'] for item in month_data)
                summary_data.append({
                    'key': f'month_{month_name}_cost',
                    'value': month_cost
                })
        
        # Save summary stats to both directories
        df_summary = pd.DataFrame(summary_data)
        df_summary.to_csv(summary_csv, index=False)
        df_summary.to_csv(service_summary_csv, index=False)
        print(f"Cost summary statistics saved to {summary_csv} and {service_summary_csv}")
        
        # Create README files with instructions for debugging
        for output_dir in [org_output_path, service_output_path]:
            ...
            with open(readme_file, 'w') as f:
                ...
                f.write("1. all_accounts_raw_data_*.csv - Consolidated data for all accounts and services\n")
                f.write("2. account_*_raw_data_*.csv - Individual account raw data files\n")
                f.write("3. account_*_monthly_totals_*.csv - Monthly totals for each account\n")
                f.write("4. organization_monthly_totals_*.csv - Organization-wide monthly totals\n")
                f.write("5. cost_summary_stats_*.csv - Summary statistics\n\n")
                f.write("Debugging tips:\n")
                f.write("- Compare 'organization_monthly_totals_*.csv' with AWS Console reports\n")
                f.write("- Check if any services are missing from the raw data\n")
                f.write("- Verify if refunds are being applied correctly\n")
                f.write("- Ensure blended vs. unblended costs are consistent with AWS Console settings\n")
        
        # We also add a special CSV that groups by service across all accounts
        service_breakdown_csv = os.path.join(service_output_path, f"service_breakdown_{start_date}_{end_date}.csv")
        ...
        if consolidated_data:
            ...
            # Save service breakdown
            df_service = pd.DataFrame(service_breakdown)
            df_service.to_csv(service_breakdown_csv, index=False)
            print(f"Service breakdown saved to {service_breakdown_csv}")
        
        print(f"Raw cost data export completed")
        return account_csv_files
    ...
    def run(self):
        ...
        # Export raw cost data first (for debugging)
        raw_data_time_start = time.time()
        self.export_raw_cost_data(accounts, date_ranges)
        raw_data_time_end = time.time()
        print(f"Raw cost data export completed in {raw_data_time_end - raw_data_time_start:.1f} seconds")
        ...
        # Generate organization service summary
        service_time_start = time.time()
        self.generate_organization_service_summary(accounts, date_ranges)
        service_time_end = time.time()
        print(f"Organization service summary completed in {service_time_end - service_time_start:.1f} seconds")
