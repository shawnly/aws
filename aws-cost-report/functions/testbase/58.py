...
def get_aws_cost_data(start_date, end_date):
    ...
    try:
        ...
        # First, try a specialized query just for refunds
        print("Performing specialized query for refunds...")
        try:
            refund_filter = {
                "Or": [
                    {
            ...
            print("Refund query successful.")
        except Exception as e:
            print(f"Refund query failed: {str(e)}")
            refund_response = {"ResultsByTime": []}
        
        # Now get regular cost data
        print("Retrieving regular cost data...")
        ...
        # Process regular cost data 
        cost_data = []
        
        # First, process refund data if any
        refund_count = 0
        for result in refund_response.get('ResultsByTime', []):
            ...
            for group in result.get('Groups', []):
                ...
                refund_count += 1
                print(f"  Specific Refund Query Found: {service}, ${cost:.2f} in period {period}")
                
                cost_data.append({
                    'Service': f"Refund: {service}",  # Mark explicitly as refund
                    'Account': account,
                    'Period': period,
                    'Cost': cost,
                    'Unit': unit
        ...
        print(f"Found {refund_count} refund entries in specialized query.")
        
        # Then process regular data
        for result in response['ResultsByTime']:
            ...
            for group in result['Groups']:
                ...
                # Add more debugging for refunds
                if cost < 0:
                    print(f"  Found negative cost: {service}, ${cost:.2f} in period {period}")
                ...
                # For potential refunds, mark them clearly
                if is_refund:
            ...
            # Check for zero-cost entries or missing entries in any period
            if not result['Groups']:
                print(f"  Warning: No data found for period: {period} to {end}")
        ...
        print(f"Retrieved data for {len(df['Service'].unique())} services across {len(df['Account'].unique())} accounts.")
        ...
        if not refund_check.empty:
            refund_by_month = refund_check.groupby('Period')['Cost'].sum()
            print("\nRefund totals by month:")
            for month, total in refund_by_month.items():
