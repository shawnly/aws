...
def get_aws_cost_data(months_back=6):
    """Fetch real AWS Cost Explorer data"""
    ...
    try:
        # Get cost by service
        response_by_service = ce_client.get_cost_and_usage(
            TimePeriod={
        ...
        # Get cost by account
        response_by_account = ce_client.get_cost_and_usage(
            TimePeriod={
                'Start': start_date_str,
                'End': end_date_str
            },
        ...
        account_data = []
        for result in response_by_account['ResultsByTime']:
            period = result['TimePeriod']['Start']
        ...
        print(f"Retrieved data for {len(service_df['Service'].unique())} services " +
              f"and {len(account_df['Account'].unique())} accounts.")
        
        return service_df, account_df, AWS_SERVICE_COLORS
        ...
        return generate_mock_data(months_back)
...
def generate_mock_data(months_back=6):
    ...
    services = list(AWS_SERVICE_COLORS.keys())
...
def create_cost_dashboard(service_df, account_df, service_colors, months_back=6, use_real_data=True):
    """Create an interactive HTML cost dashboard"""
    # Create pivot tables
    ...
    with pd.ExcelWriter(excel_path) as writer:
        ...
        # Also save raw data
        service_df.to_excel(writer, sheet_name='Raw_Services')
        account_df.to_excel(writer, sheet_name='Raw_Accounts')
    
    print(f"Report data saved to Excel: {excel_path}")
    
    # Create interactive Plotly dashboard with multiple charts
    data_source = "AWS Cost Explorer API" if use_real_data else "Mock Data (for testing)"
    title_text = f"AWS Cost Explorer Dashboard ({data_source})"
    ...
    if len(stack_data) > top_n_services:
        ...
        other_row = pd.DataFrame([other_services], index=['Other'])
...
def main():
    # Set to True to use real AWS Cost Explorer data, False for mock data
    USE_REAL_DATA = True
    MONTHS_BACK = 6
    
    try:
        if USE_REAL_DATA:
            # Try to use real AWS Cost Explorer data
            service_df, account_df, service_colors = get_aws_cost_data(MONTHS_BACK)
        else:
            # Use mock data for testing
            service_df, account_df, service_colors = generate_mock_data(MONTHS_BACK)
        
        # Create the dashboard
        create_cost_dashboard(service_df, account_df, service_colors, MONTHS_BACK, USE_REAL_DATA)
