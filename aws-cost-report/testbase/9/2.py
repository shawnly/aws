...
def parse_arguments():
    ...
    parser.add_argument('--s3-bucket', required=True, help='S3 bucket name to store reports')
    parser.add_argument('--s3-prefix', default='aws-cost-reports', help='S3 key prefix (default: aws-cost-reports)')
...
def get_s3_key_prefix(s3_prefix, account_id, end_date):
    """Get the S3 key prefix for saving files"""
    ...
    if account_id == "organization_summary":
        return f"{s3_prefix}/organization_summary/{year}/{month}"
    else:
        return f"{s3_prefix}/{account_id}/{year}/{month}"

def upload_to_s3(s3_client, bucket_name, s3_key, file_content, content_type='application/octet-stream'):
...
def create_cost_visualization(df, title, account_id, start_date, end_date, s3_client, bucket_name, s3_prefix, is_organization=False):
    """Create AWS Cost Explorer style visualization and upload to S3"""
    ...
    # Upload to S3
    s3_key_prefix = get_s3_key_prefix(s3_prefix, account_id, end_date)
    chart_s3_key = f"{s3_key_prefix}/aws-cost-chart-{account_id}-{start_date}_to_{end_date}.png"
    
    success = upload_to_s3(s3_client, bucket_name, chart_s3_key, img_buffer.getvalue(), 'image/png')
    if success:
        print(f"Cost visualization uploaded to s3://{bucket_name}/{chart_s3_key}")
    
    return chart_s3_key if success else None

def save_to_excel_s3(df, account_id, start_date, end_date, s3_client, bucket_name, s3_prefix):
    """Save DataFrame to Excel file and upload to S3"""
    ...
    # Create S3 key
    s3_key_prefix = get_s3_key_prefix(s3_prefix, account_id, end_date)
    excel_s3_key = f"{s3_key_prefix}/aws-cost-report-{account_id}-{start_date}_to_{end_date}.xlsx"
    
    # Upload to S3
    success = upload_to_s3(s3_client, bucket_name, excel_s3_key, excel_buffer.getvalue(), 
                          'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    
    if success:
        print(f"Report uploaded to s3://{bucket_name}/{excel_s3_key}")
        return excel_s3_key
    
    return None

def save_organization_summary_s3(df, start_date, end_date, s3_client, bucket_name, s3_prefix):
    """Save organization summary to Excel file and upload to S3"""
    ...
    # Create S3 key
    s3_key_prefix = get_s3_key_prefix(s3_prefix, "organization_summary", end_date)
    excel_s3_key = f"{s3_key_prefix}/aws-organization-summary-{start_date}_to_{end_date}.xlsx"
    ...
    if success:
        print(f"Organization summary report uploaded to s3://{bucket_name}/{excel_s3_key}")
        return excel_s3_key
...
def main():
    ...
    save_organization_summary_s3(org_df, start_date, end_date, s3_client, bucket_name, s3_prefix)
    ...
    if account_id:
        ...
        save_to_excel_s3(df, account_id, start_date, end_date, s3_client, bucket_name, s3_prefix)
    ...
    print("All reports and visualizations uploaded to S3 successfully!")
